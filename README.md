...
生成前にrobots.txtを確認し、HTTPエラー、HTML構造の変化、件数不足、重複URL、古すぎる記事、壊れたXMLを検出した場合は失敗する。2本とも検証を通るまで既存XMLは置き換えない。
...
`.github/workflows/update-rss.yml` は日本時間の毎時17分・47分（30分間隔）と手動実行で動く。
